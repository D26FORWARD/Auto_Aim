...
#include "robot_struct.hpp"
#include "Number_DNN.hpp"
...
struct armor_test
{
    int color;
    std::string number;
    cv::Point2f armor_pt4[4];
    float distance_to_image_center;
    Eigen::Matrix<double ,3,1> cam_pos;
    Eigen::Quaterniond q;
};
class ArmorDetector : public robot_state {
    ...
    public:
        ...
        std::vector<int> Unattack_ID_list = {6};              // 不击打列表
        int Priority_ID_list[8] = {7,1,3,4,5,6,8,2};            // 优先级列表
        int enemy_color;                                        // 选择击打颜色 1:Red 2:Blue | 电控传参
        int Base_Strike;                                        // 基地击打标识位
        std::vector<Armor_hj> Detection(const cv::Mat &src);       //识别流程入口函数
        std::vector<armor_test> Solve2pub(std::vector<Armor_hj>& Armors);
    private:
        ...
        std::vector<Armor_hj> candidateArmors; // 筛选的装甲板容器
        std::vector<Armor_hj> finalArmors;     // 最终装甲板容器
        std::vector<cv::Mat> numROIs;       // 数字ROI容器
        Armor_hj finalArmor;                   // 最终装甲板
        ...
        // =====================装甲板分数=====================
        int armorGrade(const Armor_hj& checkArmor);

        // ======================数字图像=====================
        void preImplement(Armor_hj& armor);        //获取装甲板数字图像ROI
