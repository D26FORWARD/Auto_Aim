...
public:
    ...
    Eigen::Vector3d pixel2imu(Armor_hj &armor);                            // 像素坐标转换为世界坐标
    ...
    /** 重投影 */
    void AngleSolve_show(Armor_hj &armor);                                 // 绘制装甲板朝向角
    double Armor_Angle(Armor_hj &armor);                                   // 计算装甲板朝向角度
    double World_projection(Armor_hj &armor,double Angle);                 // 计算重投影后装甲板4点距离差
    //** 枪管角度解算 */
    ...
    Eigen::Vector3d pixel2cam(Armor_hj &armor);                    // 像素坐标系转换为相机坐标系
    float calculateDistanceToCenter(const cv::Point2f & image_point);
    // Eigen::Quaterniond pnpSolve_rvec(cv::Point2f *p, int type,Eigen::Matrix<double, 3, 1>& R);
private:
    //========================参数部分========================
