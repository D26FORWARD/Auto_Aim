...
void ArmorDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  Detector_hj.enemy_color = BLUE;
  std::vector<Armor_hj> armors_hj = Detector_hj.Detection(img);

  std::vector<armor_test> armors_test = Detector_hj.Solve2pub(armors_hj);
  ...
  for (const auto & armor_tt : armors_test)
  {
    auto_aim_interfaces::msg::Armor armor_msg;
    armor_msg.number = armor_tt.number;
    armor_msg.pose.position.x = armor_tt.cam_pos(0);
    armor_msg.pose.position.y = armor_tt.cam_pos(1);
    armor_msg.pose.position.z = armor_tt.cam_pos(2);
    armor_msg.pose.orientation.x = armor_tt.q.x();
    armor_msg.pose.orientation.y = armor_tt.q.y();
    armor_msg.pose.orientation.z = armor_tt.q.z();
    armor_msg.pose.orientation.w = armor_tt.q.w();

    armor_msg.distance_to_image_center = armor_tt.distance_to_image_center;

    armor_msg.kpts.clear();
    for (int i = 0; i < 4; ++i) {
      // 修正命名空间
      geometry_msgs::msg::Point pointt;
      pointt.x = (float)armor_tt.armor_pt4[i].x;
      pointt.y = (float)armor_tt.armor_pt4[i].y;
      // std::cout<<"armor_tt.armor_pt4[i].x: "<<(float)armor_tt.armor_pt4[i].x<<std::endl;
      pointt.z = 0.0;
      // 可以在这里对 pointt 进行其他操作，比如添加到向量中
      armor_msg.kpts.emplace_back(pointt);
    }

    armor_marker_.id++;
    armor_marker_.scale.y = armor_msg.type == "small" ? 0.135 : 0.23;
    armor_marker_.pose = armor_msg.pose;
    text_marker_.id++;
    text_marker_.pose.position = armor_msg.pose.position;
    text_marker_.pose.position.y -= 0.1;
    text_marker_.text = armor_msg.type;
