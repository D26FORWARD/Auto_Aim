...
#include "../include/armor_detector/AngleSolve.hpp"

//================================内部封装函数================================
using namespace Robot;
AngleSolve::AngleSolve() {

    cv::FileStorage fs("/home/hj/rm_auto_aim_25/src/auto_aim_bringup/config/Solve_data.yaml", cv::FileStorage::READ);
...
//        std::cout << "BIG" << std::endl;
    ...
    //     {w/2, h/2, 0}     // 右下
    // };
    double half_w = (type == 1 ? big_w : small_w) ;  // 单位改为米
    double half_h = (type == 1 ? big_h : small_h) ;
...
// };
    ...
    pu.push_back(p[0]);                          //左下
    // pu.clear();
    // pu.emplace_back(p[0]);  // 左下
    // pu.emplace_back(p[3]);  // 左上
    // pu.emplace_back(p[2]);  // 右上
    // pu.emplace_back(p[1]);  // 右下
 ...
 */
Eigen::Vector3d AngleSolve::pixel2cam(Armor_hj &armor) {
    // 传入装甲板的坐标和类型
    armor.camera_position = pnpSolve(armor.armor_pt4,armor.id,armor.R);
 ...
 */
Eigen::Vector3d AngleSolve::pixel2imu(Armor_hj &armor) {
 ...
 */
double AngleSolve::World_projection(Armor_hj &armor, double Angle) {
    /** 世界坐标系转换 */
    ...
    double Armor_H_sin15;                                   // 装甲板在倾斜15度时投影宽度
    double Armor_H_cos15;                                   // 装甲板在倾斜15度时投影高度
    double cos_Armor_H_sin15;                               // 计算x轴方向投影
    double sin_Armor_H_sin15;                               // 计算y轴方向投影
    double Armor_cos_r;
    double Armor_sin_r;

    // ID判断大小装甲板
    if(armor.id != 1){
        Armor_H_sin15 = sin15*(small_h/2);               // 装甲板在倾斜15度时投影宽度
        Armor_H_cos15 = cos15*(small_h/2);               // 装甲板在倾斜15度时投影高度

        cos_Armor_H_sin15 = Armor_H_sin15*cos(Yaw);      // 计算x轴方向投影
        sin_Armor_H_sin15 = Armor_H_sin15*sin(Yaw);      // 计算y轴方向投影

        Armor_cos_r = cos(Yaw)*(small_w/2);              // 装甲板宽在x轴投影
        Armor_sin_r = sin(Yaw)*(small_w/2);              // 装甲板宽在y轴投影
    }
    else if(armor.id == 1){
        Armor_H_sin15 = sin15*(big_h/2);                 // 装甲板在倾斜15度时投影宽度
        Armor_H_cos15 = cos15*(big_h/2);                 // 装甲板在倾斜15度时投影高度

        cos_Armor_H_sin15 = Armor_H_sin15*cos(Yaw);      // 计算x轴方向投影
        sin_Armor_H_sin15 = Armor_H_sin15*sin(Yaw);      // 计算y轴方向投影

        Armor_cos_r = cos(Yaw)*(big_w/2);                // 装甲板宽在x轴投影
        Armor_sin_r = sin(Yaw)*(big_w/2);                // 装甲板宽在y轴投影
    }
 ...
 */
double AngleSolve::Armor_Angle(Armor_hj &armor) {
 ...
 *  功能: 在传入装甲板上绘制坐标系(调试)
 */
void AngleSolve::AngleSolve_show(Armor_hj &armor) {
    // 装甲板中心世界坐标
...
// std::vector<Eigen::Vector3d> AngleSolve::pnpSolveCorners(cv::Point2f *p, int type, Eigen::Matrix<double, 3, 1>& R)
// {
//     std::vector<cv::Point3d> ps;        // 装甲板在世界坐标系下的四个角点
//     std::vector<cv::Point2f> pu;        // 对应的像素坐标
//     double w;                         // 装甲板宽度
//     double h;                         // 装甲板高度
...
//     //=======================初始化平移与旋转=======================
//     // 这里 tv 为平移向量，rv 为旋转矩阵（成员变量）
//     tv = Eigen::Vector3d::Zero();
//     rv = Eigen::Matrix<double,3,3>::Zero();

//     //=======================构造世界坐标系下的四个角点=======================
//     // 假设物体坐标系下装甲板平面 z = 0，四角点依次为：左上、右上、右下、左下
//     ps = {
//             {-w / 2, -h / 2, 0.},   // 左上
//             { w / 2, -h / 2, 0.},   // 右上
//             { w / 2,  h / 2, 0.},   // 右下
...
//     //=======================构造像素坐标=======================
//     // 根据你当前的顺序：输入数组 p[0]~p[3]可能需要重新排列
//     // 这里参考原代码：输入顺序为 p[3]:左上, p[2]:右上, p[1]:右下, p[0]:左下
//     pu.push_back(p[3]);  // 左上
//     pu.push_back(p[2]);  // 右上
//     pu.push_back(p[1]);  // 右下
//     pu.push_back(p[0]);  // 左下
...
//     cv::cv2eigen(rv_mat, rv);
//     cv::cv2eigen(rvec, R); // 这里 R 保存旋转向量，也可根据需要返回其他信息
//     cv::cv2eigen(tvec, tv);

//     //=======================计算四个角点在相机坐标系下的位置=======================
//     std::vector<Eigen::Vector3d> camPoints;
//     for (size_t i = 0; i < ps.size(); i++) {
//         // 将每个角点转换为Eigen::Vector3d
//         Eigen::Vector3d pt(ps[i].x, ps[i].y, ps[i].z);
//         // 坐标变换：相机坐标 = 旋转矩阵 * 物体坐标 + 平移向量
//         Eigen::Vector3d cam_pt = rv * pt + tv;
//         camPoints.push_back(cam_pt);
//     }

//     // 可选：调试显示，类似原代码的 pnp_check 显示（省略或根据需要添加）

//     return camPoints;
// }

// std::vector<Eigen::Vector3d> AngleSolve::pixel2cam_ob(Armor &armor) {
//     // 传入装甲板的坐标和类型
//     std::vector<Eigen::Vector3d> armor_cam_pt4;
//     armor_cam_pt4 = pnpSolveCorners(armor.armor_pt4,armor.id,armor.R);
//     return armor_cam_pt4;
// }
...
//    double dist = ypd[2];
//
//    double x = dist * std::cos(pitch) * std::sin(yaw);
//    double y = dist * std::sin(pitch);
//    double z = dist * std::cos(pitch) * std::cos(yaw);
//    return {x, y, z};
//}

float AngleSolve::calculateDistanceToCenter(const cv::Point2f & image_point)
{
    // 从 C_MAT 中获取光心坐标（假设 C_MAT 已正确读取）
    float cx = C_MAT.at<double>(0, 2);
    float cy = C_MAT.at<double>(1, 2);
    return cv::norm(image_point - cv::Point2f(cx, cy));
}
// Eigen::Quaterniond AngleSolve::pnpSolve_rvec(cv::Point2f *p, int type,Eigen::Matrix<double, 3, 1>& R)
// {
//     std::vector<cv::Point3d> ps;        // 世界坐标系
//     std::vector<cv::Point2f> pu;        // 像素坐标系
//     double w;                           // 宽度
...
//     cv::cv2eigen(rv_mat,rv);
//     Eigen::Matrix3d Q_R;
//     cv::cv2eigen(rv_mat, R);

// // 利用 Eigen 的 AngleAxisd 构造四元数
//     Eigen::Quaterniond q(Q_R);
