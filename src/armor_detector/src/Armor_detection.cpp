...
ArmorDetector::ArmorDetector()
{
 ...
 //   FileStorage fs(package_path + "/SetParam_File/Detect_data.yaml", FileStorage::READ);
    ...
    armor_max_offset_angle = 30;
    armor_height_offset = 1;
    armor_ij_min_ratio = 0.4;
    armor_ij_max_ratio = 2.5;
    armor_max_angle = 45.0;
 ...
 */
void ArmorDetector::preImplement(Armor_hj& armor)
{
    Mat numDst;
    Mat num;
 ...
 */
int ArmorDetector::armorGrade(const Armor_hj& checkArmor)
{
    ////////////////////id优先级打分项目////////////////////////
    // 临时列表 1:英雄 2:工程 3|4:步兵 5:备用步兵 6:哨兵 7:前哨站 8:基地
    int id_grade;
 ...
 */
vector<Armor_hj> ArmorDetector::Detection(const cv::Mat &src)
{
    //初始化清空所有容器
...
vector<armor_test> ArmorDetector::Solve2pub(vector<Armor_hj>& Armors)
{
    vector<armor_test> results;
    AngleSolve AS;
    ...
    if (!Armors.empty()) {
        ...
        for (auto &armor : Armors) {
            armor_test result;
            // 装甲板转换到相机坐标系
            Armor_hj armorCopy = armor;  // 如有需要对 armor 修改，则创建副本
            armorCopy.camera_position = AS.pixel2cam(armor);
            // 计算距离图像中心的距离
            float distance = AS.calculateDistanceToCenter(armor.center);
            // PnP求解旋转向量转为四元数
            Eigen::Vector3d rvec = armor.R;  // 旋转向量，单位是“轴 × 角度”
            // double angle = rvec.norm();          // 旋转角（弧度）
            // Eigen::Quaterniond q = (angle < 1e-6)
            // ? Eigen::Quaterniond::Identity()
            // : Eigen::Quaterniond(Eigen::AngleAxisd(angle, rvec.normalized()));
            
            // rvec 是 Eigen::Vector3d
            cv::Mat rvec_cv;
            cv::eigen2cv(rvec, rvec_cv);

            // 转为旋转矩阵
            cv::Mat rotation_matrix;
            cv::Rodrigues(rvec_cv, rotation_matrix);
            ...
            tf2::Quaternion tf_q;
            tf2_R.getRotation(tf_q);

            // 转为 Eigen 四元数（如果你要存到 result.q）
            Eigen::Quaterniond q(tf_q.w(), tf_q.x(), tf_q.y(), tf_q.z());
        

            // // 计算绕y轴旋转90度的四元数
            // double angle_y = -M_PI / 2.0;
            // Eigen::Quaterniond q_y90(std::cos(angle_y / 2.0), 0, std::sin(angle_y / 2.0), 0);

            // // 计算绕z轴旋转 90 度的四元数
            // double angle_z = -M_PI / 2.0;
            // Eigen::Quaterniond q_z90(std::cos(angle_z / 2.0), 0, 0, std::sin(angle_z / 2.0));

            // Eigen::Quaterniond q_f(0.5, 0.5, 0.5, 0.0);

            // // 修正四元数
            // Eigen::Quaterniond q_fix = q_y90 * q;

            result.q = q;


            // 填充结果数据
            result.number = std::to_string(armor.id);
            result.distance_to_image_center = distance;
            result.cam_pos = armorCopy.camera_position;
            
            result.color = 0;
    
            result.armor_pt4[0] = armorCopy.armor_pt4[3];
            result.armor_pt4[1] = armorCopy.armor_pt4[0];
            result.armor_pt4[2] = armorCopy.armor_pt4[1];
            result.armor_pt4[3] = armorCopy.armor_pt4[2];

            // result.armor_pt4[0] = armorCopy.armor_pt4[0];
            // result.armor_pt4[1] = armorCopy.armor_pt4[1];
            // result.armor_pt4[2] = armorCopy.armor_pt4[2];
            // result.armor_pt4[3] = armorCopy.armor_pt4[3];
